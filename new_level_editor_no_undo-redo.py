...
import os
import time
import copy  # <-- IMPORTANTE para copiar diccionarios
        ...
        if change == QGraphicsItem.ItemPositionChange and self.scene():
            ...
            if self.editor and self.editor.chk_grid_snap.isChecked():
                ...
                if grid_size > 0:
                    ...
                    # IMPORTANTE: Retornamos la posición ajustada a la rejilla
                    # Esto fuerza a Qt a dibujar el objeto en la posición del grid
                    if change == QGraphicsItem.ItemPositionChange:
                        # Actualizamos data (centrado)
                        scale = self.scale()
...
class LevelEditor(QMainWindow, Ui_LevelEditor):
    def __init__(self):
        ...
        self.clipboard_data = None # Variable para Copiar/Pegar
    ...
    def browse_file_for_combo(self, combo_widget):
        ...
        if filepath:
            try:
                ...
                print("Error: El archivo debe estar dentro de la carpeta del proyecto.")
    ...
    def copy_object(self):
        ...
        # Hacemos una copia profunda (Deep Copy) del diccionario
        # para que no sea una referencia viva
        original_data = current_item.data(Qt.UserRole)
        self.clipboard_data = copy.deepcopy(original_data)
        print(f"Copiado al portapapeles: {self.clipboard_data.get('id')}")
    ...
    def paste_object(self):
        ...
        # Crear nuevos datos basados en el portapapeles
        new_obj_data = copy.deepcopy(self.clipboard_data)
        
        # Generar nuevo ID único
        new_id = f"obj_{int(time.time())}_{len(self.current_data['zones'][current_zone_key])}"
        new_obj_data['id'] = new_id
        
        # Desplazar ligeramente para que se vea
        new_obj_data['x'] += 20
        new_obj_data['y'] += 20
        
        # Añadir a los datos de la zona
        ...
        item_text = f"[{new_obj_data.get('type', 'Obj')}] {new_id}"
        list_item = QListWidgetItem(item_text)
        list_item.setData(Qt.UserRole, new_obj_data)
        self.list_objects.addItem(list_item)
        
        # Dibujar en Canvas
        pix_item = self.draw_object_on_canvas(new_obj_data)
        if pix_item:
            list_item.setData(Qt.UserRole + 1, pix_item)
            
        # Seleccionar el nuevo objeto pegado
        self.list_objects.setCurrentItem(list_item)
        print(f"Pegado objeto: {new_id}")
    ...
    def navigate_zone(self, dx, dy):
        ...
        if new_y < 0 or new_y >= len(WORLD_MAP_LEVEL) or new_x < 0 or new_x >= len(WORLD_MAP_LEVEL[0]): return
        if WORLD_MAP_LEVEL[new_y][new_x] == 0: return
        ...
        if index != -1:
            ...
            print(f"Creando zona: {target_zone_key}")
