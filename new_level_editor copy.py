...
# --- CLASES VISUALES CON PROTECCIÓN DE MOVIMIENTO ---
...
class ResizeHandle(QGraphicsRectItem):
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.ItemPositionChange and self.scene():
            # Si el padre está siendo movido por código, no hacemos nada
            ...
            # Protección contra división por cero o pixmap vacío
...
class HitboxItem(QGraphicsRectItem):
    def __init__(self, rect, obj_data, parent_sprite_item, editor_ref):
        ...
        self.ignore_movement = False # BANDERA DE SEGURIDAD
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.ItemPositionChange and self.scene() and self.parent_sprite:
            # CRÍTICO: Si el código está moviendo esto, NO recalcular offset
...
class LevelEditor(QMainWindow, Ui_LevelEditor):
    def __init__(self):
        ...
        # Banderas para control de flujo y evitar bucles
        ...
        # Importante: Esta señal maneja la lógica principal de rellenar datos
    ...
    # --- SINCRONIZACIÓN VISUAL (CANVAS -> UI) ---

    def on_scene_selection_changed(self):
        """Se ejecuta al hacer click en el Canvas."""
        # Si estamos actualizando la selección desde la lista, ignoramos esto
        # para evitar conflictos, PERO no bloqueamos la lógica si es click real.
        if self.list_objects.signalsBlocked():
            return
        ...
        if target_data:
            ...
            # Buscar por ID para ser precisos
            for i in range(self.list_objects.count()):
                ...
                if item_data.get("id") == target_id:
                    ...
                    self.list_objects.scrollToItem(list_item) # Asegurar visibilidad
                    self._updating_selection_from_canvas = False
                    
                    # Forzamos la actualización del panel
    ...
    def update_hitbox_position_only(self):
        """Mueve el hitbox visualmente sin recrearlo."""
        if not self.current_hitbox_item or not self.list_objects.currentItem(): return
        
        # Usamos la bandera para que este movimiento NO dispare itemChange lógica
    ...
    def draw_object_on_canvas(self, obj):
        ...
        # Creamos el item con ignore_movement=False por defecto, pero...
        item = LevelObjectItem(scaled, obj, self)
        # ...al setear posición inicial, lo marcamos para que no dispare logica de guardado
    ...
    def on_object_selected(self, current, previous):
        """
        Esta función se llama siempre que cambia la selección en la lista.
        Ya sea por click en lista O por click en canvas (vía setCurrentItem).
        """
        if not current:
            self.disable_property_panel()
            return
